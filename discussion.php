...
<body>
    ...
    <div class="container">
        <div class="row">
            <?php include 'con_db.php'; $cdate=date('Y-m-d');
                $sql=mysqli_query($con,"SELECT * FROM loopholes");
             ...
             <div class="col-md-4">
                 <form action="insert.php" method="post">
                   <div class="form-group">
                     <textarea class="form-control" name="content" placeholder="Start the discussion"></textarea>
                   </div>
             ...
             <?php  } else{ ?>
                <div class="row">
                    <div class="col-md-12">
                        <button class="btn btn-info" onclick="showform();">Create New Discussion</button>
                    </div>
                </div><br><br>
                <div class="row">
                  <div class="col-md-4">
                        <div id="displayForm" style="width: 500px;"></div>
                  </div>
                </div><br><br>
                    ...
                    <tbody>
                        <?php include('con_db.php');
                            $i=1; $tdate=date('Y-m-d');
                                $qry=mysqli_query($con,"SELECT * from loopholes")or die(mysqli_error($con));
                                while ($row=mysqli_fetch_array($qry)){
                            ?>
                                <tr>
                                    <td><?php echo $i++; ?></td>
                                    <td><?php echo $row['content']; ?></td>
                                    <td><?php $m_id=$row['mid']; if($m_id==$mid){ echo 'Me';} else{ echo $row['mname'];}  ?></td>
                                    <td><?php echo $row['datetime']; ?></td>
                                    <td><?php $m_id=$row['mid']; if($m_id==$mid){ ?>
                                        <a href="view_replies.php?lid=<?php echo $row['lid']; ?>" class="btn btn-warning" title="View reply"><i class="fa fa-eye"></i></a> <a href="reply.php?lid=<?php echo $row['lid']; ?>" class="btn btn-danger" title="Reply with content"><i class="fa fa-envelope"></i></a>
                                        <?php } else {?>
                                        <a href="reply.php?lid=<?php echo $row['lid']; ?>" class="btn btn-danger" title="Reply with content"><i class="fa fa-envelope"></i></a> <a href="view_replies.php?lid=<?php echo $row['lid']; ?>" class="btn btn-warning" title="View reply"><i class="fa fa-eye"></i></a>
                                        <?php } ?>
                                        </td>
                                </tr>
                                <?php }?>
    ...
    <script type="text/javascript">
        function showform()
        {
            var html= '<form action="insert.php" method="post"><div class="form-group"><textarea class="form-control" name="content" placeholder="Start the discussion"></textarea></div><input type="submit" class="btn btn-success" name="save"></form><br/><br/>';
