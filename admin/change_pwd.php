    ...
    <section class="about contact py-lg-5 py-md-5 py-3">
        ...
        <?php include('con_db.php');
            if(isset($_POST['change']))
            {
                ...
                $npwd=$_POST['npwd'];
                $rpwd=$_POST['cpwd'];
                $sql=mysqli_query($con,"select * from admin where password='$opwd' and username='$u_name'")or die(mysqli_error($con));
                $nrows=mysqli_num_rows($sql);
                if($nrows==1)
                {
                    if($npwd==$rpwd)
                    {
                        $qry=mysqli_query($con,"update admin set password='$npwd' where username='$u_name'"); 
                        if($qry)
                        {
